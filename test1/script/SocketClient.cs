...
//---------------------------------------- 外部调用 -------------------------------------------------
    ...
    // 发消息给服务器
    public void SendMessage(int maincmd, UInt16 childcmd, UInt16 token, byte[] buffer)
    {
        try
        {
//            var sendBuffer = TCPHeader.GetSendTcpHeaderData((UInt16) maincmd, childcmd, token,System.Text.Encoding.Default.GetBytes(buffer));
            var sendBuffer = TCPHeader.GetSendTcpHeaderData((UInt16) maincmd, childcmd, token,buffer);
...
//            PrintBytes(sendBuffer);
    ...
    [CSharpCallLua]
    public delegate void FDelegateNetWorkReceive(int a, int b, byte[] c, string d);
